...
class _StarRatingState extends State<StarRating> {
  ...
  Widget build(BuildContext context) {
    return Stack(
      children: <Widget>[         
        Row(children: _getUnSelcetedStar(), mainAxisSize: MainAxisSize.min),
        Row(children: _getSelcetedStar(), mainAxisSize: MainAxisSize.min),
      ],
    );
  }
  /// 未选中的星
  List<Widget> _getUnSelcetedStar(){
    return List.generate(widget.count, (index) => widget.unselectedImage);
  }

  /// 选中的星
  List<Widget> _getSelcetedStar(){
       // 1.计算Star个数和剩余比例等
